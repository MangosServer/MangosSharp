				...
				try
				{
					...
					if (creatureInfo.TrainerType == 0)
					{
						if ((uint)creatureInfo.Classe == (uint)objCharacter.Classe)
						{
							...
							{
								npcMenu.AddMenu("I want to unlearn all my talents.", 0, 0);
					...
					else if (creatureInfo.TrainerType == 2)
					{
						...
						{
							npcMenu.AddMenu("I am interested in professions training.", 3, 0);
							objCharacter.TalkMenuTypes.Add(Gossip_Option.GOSSIP_OPTION_TRAINER);
							goto IL_0431;
						}
						textID = 11031;
					...
					else
					{
						...
						{
							npcMenu.AddMenu("I am interested in pet training.", 3, 0);
							objCharacter.TalkMenuTypes.Add(Gossip_Option.GOSSIP_OPTION_TRAINER);
							goto IL_0431;
						}
						textID = 3620;
			...
			checked
			{
				...
				if (!client.Character.HaveSpell(spellInfo.ID) && client.Character.Copper >= spellCost && unchecked((uint)client.Character.Level >= (uint)reqLevel) && (reqSpell <= 0 || client.Character.HaveSpell(reqSpell)) && (Conversions.ToInteger(mySqlQuery.Rows[0]["reqskill"]) <= 0 || client.Character.HaveSkill(Conversions.ToInteger(mySqlQuery.Rows[0]["reqskill"]), Conversions.ToInteger(mySqlQuery.Rows[0]["reqskillvalue"]))))
				{
					try
					{
						...
						WS_Spells.SpellTargets spellTargets2 = spellTargets;
		...
		public void On_CMSG_SELL_ITEM(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
		{
			checked
			{
				...
				try
				{
					...
					foreach (KeyValuePair<byte, ItemObject> item2 in client.Character.Items)
					{
		...
		public void On_CMSG_BINDER_ACTIVATE(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
		{
			if (checked(packet.Data.Length - 1) >= 13)
			{
				...
				if (WorldServiceLocator._WorldServer.WORLD_CREATUREs.ContainsKey(guid))
				{
					...
					WS_Spells.CastSpellParameters castSpellParameters = new WS_Spells.CastSpellParameters(ref spellTargets, ref Caster, 3286, Instant: true);
